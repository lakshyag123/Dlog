...
const express = require("express"); // framework for nodejs at back end
const path = require('path');
const fs = require('fs');
const methodOverride= require("method-override");
const bodyParser = require("body-parser");  // middleware
const ejs = require("ejs");       // embeded javascript helps to write plain js on both front & back end
const mongoose = require("mongoose");      // helps to write, validate, modify mongodb files easily using schema based model
const session = require('express-session');  //stores user information like cookies untill browser is open(Server-side)
...
const multer  = require('multer');     //used for uploading files on db
...
app.use(express.static("public"));       // using express framework & ejs template engine 
...
mongoose.connect("mongodb+srv://Dlog_admin:dlogpassword@cluster0.2la1x.mongodb.net/dlogDB", {useNewUrlParser: true});
...
app.post("/register", function (req, res) {
  const newUser = new User({ fname: req.body.fname, username: req.body.username,gender: req.body.gender, verify: false });
...
// app.put("/blogs/:id",uploads,async (req,res)=>{
//   const {id}=await req.params;
  
//   const newblog=await Blog.findByIdAndUpdate(id,req.body, {runValidators: true,new: true});
//   console.log(newblog);
//   res.redirect('/blogs');}
